...
api_key = '22auQKbOKurVIzGq2RMrP5lmHCGENJFIyU6f4GC2TEwxvkLoWs51CDXkONU8wYyV'
api_secret = 'UGwNhkriDatEpHttqOZISzPBzVbvAJKLpCOrtgUH4RW2gS3WaBmNm2p68BidPx44'

client = Client(api_key, api_secret)

class ModelTools:

    def __init__(self, currency: str, startPeriod: str, endPeriod: str):
        self.currency = currency
        self.startPeriod = startPeriod
        self.endPeriod = endPeriod

        self.klines = client.get_klines(symbol=currency.upper(), interval=Client.KLINE_INTERVAL_1WEEK)
        self.currentPeriod = None
        self.patterns = None

        # EMA Values
        self.firstPeriod = 50
        self.secondPeriod = 99
        self.ema50 = self.getEMA(self.klines, self.firstPeriod)
        self.ema99 = self.getEMA(self.klines, self.secondPeriod)
        ...
        self.emaCrosses = self.getEmaCrosses([self.firstPeriod, self.secondPeriod], [self.ema50, self.ema99])
    ...
    def setTradeData(self):
        self.klines = client.get_historical_klines(symbol=self.currency.upper(), interval=Client.KLINE_INTERVAL_1DAY, start_str=self.startPeriod)
        ...
        self.momentums = self.getDMIMomentum(self.dmiPeriod, trendDirection)
                ...
                if res <= (downPercentage if bool(downPercentage) else 0.92) or (
                    ...
                    if valueToCompare['type'] == 'negative':
                        ...
                        values.append({
                            ...
                            downPercentage + 4 if bool(downPercentage) else 0.96) >= res > (
                    ...
                    if valueToCompare['type'] == 'positive':
                        ...
                        values.append({
                            ...
                            climbPercentage - 4 if bool(climbPercentage) else 1.08) <= res < (
    ...
    def getDMIMomentum(self, entry: list, trendDirection: int):
        ...
        if not bool(trendDirection):
            ...
            for item in entry:
                ...
                if not (pre_DI is None):
                    ...
                    if pre_DI['value'] <= adx and adx > negDI and pre_DI['date'] < item['date']:
                        if pre_ADX is None:
        ...
        if bool(trendDirection):
            ...
            for item in entry:
                ...
                if not (pre_DI is None):
                    ...
                    if pre_DI['value'] <= adx and adx > posDI and pre_DI['date'] < item['date']:
                        if pre_ADX is None:
                            pre_ADX = {'date': item['date'], 'value': adx}
                        elif pre_ADX['value'] < adx:
                            pre_ADX = {'date': item['date'], 'value': adx}
                    ...
                    if len(entry) - 1 == entry.index(item) and pre_ADX is None:
                    ...
                    if len(entry) - 1 == entry.index(item) and not(pre_ADX is None):
                        momentum.append({
                            'DI': {
                                'value': pre_DI['value'],
                                'date': pre_DI['date'],
